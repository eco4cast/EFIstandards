...
```{r}
start_time = as.Date("2001-03-04")  ## start date of our forecast
n_time <- 30
times = seq(from=start_time,by="1 day",length=n_time)
...
```{python}
start_time = datetime.date(2001, 3, 4)
n_time = 30
times = [start_time + datetime.timedelta(days = x) for x in range(n_time)]
...
timevar.units = f"days since {start_time}"
timevar[:] = [(t - start_time).days for t in times]
