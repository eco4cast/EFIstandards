...
forecastnc = ncout.createVariable("forecast", "i4", ("datetime"))
...
df_combined <- reshape2::melt(n,varnames=c("datetime","depth","parameter","obs_flag","species")) %>%
  ...
  mutate(datetime = datetimes[time]) %>%
  mutate(depth = depths[depth]) %>%
  right_join(tibble(datetime=datetimes,forecast=forecast,data_assimilation = data_assimilation))
...
dfs <- df_combined %>% add_column(family="normal") %>%
                   group_by(datetime, depth,family,obs_flag,variable,forecast,data_assimilation) %>%
...
attributes <- tibble::tribble(
 ...
 "datetime",              "[dimension]{datetime}",                          "year",                 "YYYY-MM-DD",  "numberType", NA,
